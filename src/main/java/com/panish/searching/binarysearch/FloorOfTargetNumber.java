...
public class FloorOfTargetNumber {
    ...
    private static void case1() {
        ...
        int target = 3;
        System.out.println(getFloor(ary, target));
    }
    ...
    private static int getFloor(int[] ary, int target) {

        int start = 0;
        int end = ary.length - 1;

        // Check if tarter is less than all element
        if (target < ary[start]) return -1;
